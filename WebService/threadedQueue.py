...
class Request(object):
    ...
    def __init__(self, num):
        self.time = time.time()
        self.order = Request.counter
        Request.counter += 1
        self.num = num

    def __str__(self):
        return str(self.order) + ". Request - " + str(self.num) + "               " + str(self.time)


class Result(object):
    counter = 1

    def __init__(self,reqTime, reqNum, timeF=None):
        self.timeS = time.time()
        self.order = Result.counter
        Result.counter += 1
        self.reqNum = reqNum
    ...
    def __str__(self):
        return str(self.order) + ". Result for num: " + str(self.reqNum) + \
